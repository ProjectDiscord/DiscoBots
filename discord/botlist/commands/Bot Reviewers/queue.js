        ...
        if (guild.members.cache.has(message.author.id)) {
          if (guild.members.cache.get(message.author.id).roles.cache.has(global.config.server.roles.botReviewer)) {
            ...
            for (let i = 0; i < bot.length; i++) {
              await bots2.push(`- ${bot[i].username} - ${bot[i].botID} **[ [Invite to Test](https://discord.com/oauth2/authorize?client_id=${bot[i].botID}&scope=bot&guild_id=1127093259613777950&permissions=0) ]**`);
            }
            const embed = {};
